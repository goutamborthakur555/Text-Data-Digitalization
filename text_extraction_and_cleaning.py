...
"""
############################################################
# # Extracting Text From PDF, Cleaning and Exporting to Excel File
# # Installing Libraries
# !pip install pymupdf Pillow opencv-python
# !pip install pytesseract opencv-python pillow
# !pip install pymupdf Pillow opencv-python pandas openpyxl
...
# Example sentence
sentence = export_text

# Specify the filename
filename = "CLASS_1to4_ExportedFile.txt"

# Call the function to export the sentence to the file
export_sentence_to_file(sentence, config.output_file_loc + filename)


# ## Text Cleaning
# sample_text = '''
# Page 1:
# Trade Marks Journal No: 2152 ,   15/04/2024           Class 3


# 5392752    31/03/2022
# MR. RAMESH KUMAR SANTWANI S/O. GURUMUKH DAS SANTWANI
# Ward No.25, Arjun Sitlani k Paas, Dhimar Mohalla, Dabra, Dabra,  Gwalior, Madhya Pradesh-475110, INDIA
# An Indian Proprietorship Firm
# Address for service in India/Attorney address:
# S.S. ADVOCATES & COMPANY
# 688/A/17, 2ND FLOOR, S.NO. 1387/A/17, SHREYAS PLAZA, NEAR TRAN KHUNIYA GARDEN, MIRZAPUR ROAD, AHMEDABAD -
# 380 001 - GUJARAT
# Used Since :01/12/2021
# MUMBAI
# Soap, Dish Wash Bar, Liquid Deter gent, Liquid Soap, Detergent P owder, Cleaning Powder, Dish Wash Liquid Material, Wall
# Cleaner, Marble Cleaner, Multipur pose Cleaner, Bleaching Prepar ation, Hand wash, Toilet Cleaner.
# Mark must be used as a whole and no monopoly right is given over any single term.
...
for i in range(0,len(pages_list)):
    ...
    if (("Trade Marks Journal No" in cond_1) and ("Class" in cond_1) and
        ...
        try:
            ...
            elif "/" in (raw_text.split("Class ")[1].split("\n")[applicant_cnt + 1].strip().split(" ")[-1].strip()):
                text4 = raw_text.split("Class ")[1].split("\n")[applicant_cnt + 1].strip().split(" ")[-1].strip()
                logo_name = extr_date

            else:
                text4 = raw_text.split("Class ")[1].split("\n")[applicant_cnt + 2].strip().split(" ")[-1].strip()
                logo_name = extr_date
        ...
        try:
            ...
            if "Proposed to be Used" in raw_text:
                ...
                desc = "\n".join([text14, text15, text16, text17, text18, text19, text20, text21, text22, text23,
